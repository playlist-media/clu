...
var helpEnviron = &Command{
	Usage:    "environ",
	Category: "clu",
	Short:    "environment variables used by clu",
	Long: `
Several environment variables affect clu's behavior.

CLUDEBUG

  When this is set, clu prints more verbose information about what
  is going on behind the scenes, to aid in debugging.
`,
}
...
var cmdVersion = &Command{
	...
	Usage:    "version",
	Category: "clu",
	Short:    "show clu version",
	Long:     `Version shows the clu version string.`,
}
...
Usage: clu <command> [-a <app or remote>] [options] [arguments]
...
Run 'clu help [command]' for details.
...
{{if .Dev}}This dev build of clu cannot auto-update itself.
{{end}}`[1:]))
...
func printAllUsage() {
	...
	for i := range cl {
		if cl[i].Runnable() {
			listRec(w, "clu "+cl[i].FullUsage(), "# "+cl[i].Short)
		}
	}
}
